...
const PORTFOLIO_TOOLS = [
    ...
    },
    {
        name: "get_pokemon",
        title: "Get Pokemon",
        description: "Lookup Pokemon data by name or National Dex id from local PokeAPI api-data sync (falls back to live PokeAPI when missing, cached locally)",
        schema: {
            nameOrId: "string"
        }
    },
...
function parseDexIdFromPokeApiUrl(url) {
...
async function loadLocalJson(absolutePath) {
    const raw = await readFile(absolutePath, "utf8");
    return JSON.parse(raw);
}
async function loadPokemonIndex() {
    const indexPath = path.join(pokeapiV2Dir, "pokemon", "index.json");
    const parsed = await loadLocalJson(indexPath);
    return Array.isArray(parsed.results) ? parsed.results : [];
}
async function resolvePokemonId(nameOrId) {
...
async function fetchPokemonFromPokeApi(nameOrId) {
    const trimmed = nameOrId.trim();
    const asNumber = Number(trimmed);
    const { PokemonClient } = await import("pokenode-ts");
    const client = new PokemonClient();
    if (Number.isInteger(asNumber) && asNumber > 0) {
        return (await client.getPokemonById(asNumber));
    }
    return (await client.getPokemonByName(trimmed.toLowerCase()));
}
...
function asTypeName(value) {
    ...
    return CANONICAL_TYPE_NAMES.includes(n) ? n : null;
}
function toNameList(list) {
    if (!Array.isArray(list))
        return [];
    return list.map((x) => x?.name).filter((x) => typeof x === "string").map(normalizeTypeName);
}
...
async function resolveTypeNameOrId(typeNameOrId) {
...
async function fetchTypeFromPokeApi(typeNameOrId) {
    const { PokemonClient } = await import("pokenode-ts");
    const client = new PokemonClient();
    const trimmed = typeNameOrId.trim();
    const asNumber = Number(trimmed);
    if (Number.isInteger(asNumber) && asNumber > 0) {
        return (await client.getTypeById(asNumber));
    }
    return (await client.getTypeByName(trimmed.toLowerCase()));
}
async function loadTypeDamageRelations(typeName) {
    ...
    if (resolved?.id) {
        ...
        if (await fileExists(localApiDataPath)) {
            const data = await loadLocalJson(localApiDataPath);
    ...
    if (await fileExists(cachePath)) {
        const data = await loadLocalJson(cachePath);
        const rel = data.damage_relations ?? {};
        const out = {
            double_damage_from: toNameList(rel.double_damage_from),
            half_damage_from: toNameList(rel.half_damage_from),
...
async function computeDefensiveMultiplier(attackingType, defendingTypes) {
    let multiplier = 1;
    for (const defType of defendingTypes) {
        const rel = await loadTypeDamageRelations(defType);
        if (rel.no_damage_from.includes(attackingType))
            multiplier *= 0;
        else if (rel.double_damage_from.includes(attackingType))
            multiplier *= 2;
        else if (rel.half_damage_from.includes(attackingType))
            multiplier *= 0.5;
    }
    return multiplier;
}
async function computeAllDefensiveMultipliers(defendingTypes) {
    const out = {};
...
function extractPokemonTypeNames(pokemonData) {
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Pokemon search failed. Ensure PokeAPI api-data is synced into ${pokeapiDataDir}. ` +
    ...
    try {
        ...
        if (resolved) {
            ...
            if (await fileExists(pokemonPath)) {
                const data = await loadLocalJson(pokemonPath);
                return {
                    content: [
                        {
                            type: "text",
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Pokemon lookup failed. This tool prefers local PokeAPI api-data sync, and can fall back to live PokeAPI when missing.\n\n` +
                        `Local expected: ${path.join(pokeapiV2Dir, "pokemon", "index.json")} and per-Pokemon JSON under ${path.join(pokeapiV2Dir, "pokemon", "<id>", "index.json")}.\n` +
...
server.registerTool("type_effectiveness", {
    title: "Type Effectiveness",
    description: "Calculate damage multiplier for an attacking type against 1-2 defending types.",
    inputSchema: z.object({
...
server.registerTool("counter_pokemon", {
    title: "Counter Pokemon",
    description: "Suggest best attacking types (and example Pokemon when local api-data exists) to counter a target Pokemon.",
    inputSchema: z.object({
    ...
    try {
        ...
        try {
            ...
            if (resolved) {
                ...
                if (await fileExists(localPath))
                    pokemonData = await loadLocalJson(localPath);
                else {
                    const cachePath = path.join(pokeapiCacheV2Dir, "pokemon", `${resolved.id}.json`);
                    if (await fileExists(cachePath))
                        pokemonData = await loadLocalJson(cachePath);
                }
            }
        }
        catch {
        ...
        if (!pokemonData)
            pokemonData = await fetchPokemonFromPokeApi(parsed.targetNameOrId);
        ...
        const sorted = Object.entries(mults)
            .map(([type, multiplier]) => ({ type: type, multiplier }))
        ...
        if (haveLocalIndex) {
            ...
            for (const r of index) {
                ...
                for (const t of top) {
                    if ((examples[t.type]?.length ?? 0) >= samplePerType)
                        continue;
...
server.registerTool("suggest_team", {
    title: "Suggest Team",
    ...
    try {
        ...
        for (const entry of parsed.team) {
            ...
            try {
                ...
                if (resolved?.id) {
                    const localPath = path.join(pokeapiV2Dir, "pokemon", String(resolved.id), "index.json");
                    if (await fileExists(localPath))
                        data = await loadLocalJson(localPath);
                    else {
                        const cachePath = path.join(pokeapiCacheV2Dir, "pokemon", `${resolved.id}.json`);
                        if (await fileExists(cachePath))
                            data = await loadLocalJson(cachePath);
                    }
                }
            }
            catch {
            ...
            if (!data)
                data = await fetchPokemonFromPokeApi(entry);
            members.push({
                input: entry,
        ...
        for (const m of members) {
            const mults = await computeAllDefensiveMultipliers(m.types.length ? m.types : []);
            for (const t of CANONICAL_TYPE_NAMES) {
                const v = mults[t];
                perTypeStats[t].multipliers.push(v);
                if (v === 0)
                    perTypeStats[t].immune += 1;
                else if (v >= 2)
                    perTypeStats[t].weak += 1;
                else if (v <= 0.5)
                    perTypeStats[t].resist += 1;
                else
                    perTypeStats[t].neutral += 1;
            }
        }
